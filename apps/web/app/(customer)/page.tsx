...
// Sort active jobs first, by urgency of status
...
export default function HomePage() {
  ...
  const doneJobs    = recentJobs.filter(j =>  ['completed','cancelled'].includes(j.status)).slice(0, 2)
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div style={{ background: 'linear-gradient(160deg, #060d1f 0%, #0a1e3d 100%)' }} className="px-5 pt-12 pb-20">
        <div className="relative flex items-center justify-center mb-8">
          ...
          <img
            ...
            style={{ filter: 'drop-shadow(0 2px 16px rgba(249,115,22,0.5))' }}
          />
      ...
      <div className="px-5 -mt-10 space-y-5 pb-10">
        ...
        {/* ═══════════════════════════════════════
            STATUS PERMINTAAN — PALING ATAS SEKALI
            ═══════════════════════════════════════ */}
        {user && user.role === 'customer' && (
          <>
            {jobsLoading ? (
              ...
              /* Active jobs — prominent status cards */
              <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-5">
                      ...
                      <Link key={job.id} href={`/jobs/${job.id}`}
                        ...
                        <div className="mt-1">
                          <div className="flex justify-between mb-1">
                            {STEPS.map((step, i) => (
                              <span key={step}
                                className={`text-[9px] font-black ${i < s.step ? s.color : 'text-gray-300'}`}>
                                {step}
                              </span>
                            ))}
                          </div>
                          <div className="h-1.5 bg-white/60 rounded-full overflow-hidden">
                            <div className="h-full rounded-full transition-all"
                              style={{
                                background: s.step === 0 ? '#d1d5db' : 'linear-gradient(90deg,#F97316,#EA6C0A)',
                                width: `${(s.step / 5) * 100}%`
                              }} />
                          </div>
                        </div>
                ...
                <Link href="/jobs/new"
                  className="mt-3 flex items-center justify-center gap-2 w-full py-3 rounded-2xl border-2 border-dashed border-gray-200 text-xs font-black text-gray-400 hover:border-orange-300 hover:text-orange-400 transition">
                  <span className="text-base">+</span> Post Job Baru
                </Link>
              </div>

            ) : recentJobs.length > 0 ? (
              /* All jobs done — encourage new post */
              <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-5">
                ...
                <Link href="/jobs/new"
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-2xl text-xs font-black text-white"
              ...
              <div className="rounded-3xl p-6 text-white relative overflow-hidden"
                ...
                <div className="relative">
                  ...
                  <p className="text-orange-100 text-sm mb-4 leading-relaxed">
                    Post job sekarang — Technician berdekatan akan buat tawaran harga untuk anda
                  </p>
                  <Link href="/jobs/new"
                    className="inline-flex items-center gap-2 bg-white text-[#F97316] font-black text-sm px-5 py-2.5 rounded-xl hover:bg-orange-50 transition shadow-lg">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-5 border border-gray-100">
          ...
          {categories.length === 0 ? (
            <div className="grid grid-cols-3 gap-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-gray-100 rounded-2xl p-3 animate-pulse h-16" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-3">
        ...
        {/* ═══════════════════
            PERMINTAAN SAYA — quick link
            ═══════════════════ */}
        {user && (
          <Link href="/jobs"
            className="flex items-center gap-3 bg-white rounded-3xl shadow-sm border border-gray-100 p-4 hover:bg-gray-50 active:scale-[0.99] transition-all">
        ...
        {/* CTA Banner — hanya kalau belum login */}
        {!user && (
          <div className="rounded-3xl p-6 text-white overflow-hidden relative" style={{ background: 'linear-gradient(135deg, #F97316, #EA6C0A)' }}>
            ...
            <div className="relative">
              ...
              <p className="text-orange-100 text-sm mb-4 leading-relaxed">
                Post job sekarang, Technician berdekatan akan hubungi anda
              </p>
              <Link href="/login"
                className="inline-flex items-center gap-2 bg-white text-[#F97316] font-black text-sm px-5 py-2.5 rounded-xl hover:bg-orange-50 transition shadow-lg">
        ...
        <div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-8 text-center shadow-sm">
            ...
            <p className="text-sm font-semibold text-gray-700 mb-1">Aktifkan Lokasi</p>
            <p className="text-xs text-gray-400">Untuk lihat Technician yang berdekatan dengan anda</p>
          </div>
        </div>
