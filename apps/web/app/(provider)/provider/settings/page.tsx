...
export default function ProviderSettingsPage() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const providerFields: Record<string, string | number> = { business_name: businessName }
      if (bio) providerFields.bio = bio
      if (yearsExp !== '') providerFields.years_experience = Number(yearsExp)
      await api.patch('/api/providers/me', providerFields)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div style={{ background: 'linear-gradient(160deg,#060d1f 0%,#0a1e3d 100%)' }} className="px-5 pt-10 pb-16">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/provider/dashboard')} className="text-white/60 hover:text-white transition">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div>
            <p className="text-blue-200 text-xs font-semibold tracking-widest uppercase mb-0.5">Technician</p>
      ...
      <div className="px-5 -mt-8 pb-10 space-y-4">
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6 flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center font-black text-white text-2xl shrink-0"
          ...
          <div>
            ...
            <span className="inline-block mt-1 text-xs font-bold px-2.5 py-0.5 rounded-full bg-orange-50 text-[#F97316]">
              Technician
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 space-y-4">
          <h2 className="text-xs font-black text-gray-500 uppercase tracking-widest">Maklumat Profil</h2>
          ...
          <div>
            <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Nama Perniagaan</label>
            <input value={businessName} onChange={e => setBusinessName(e.target.value)} type="text"
              placeholder="cth: Ali Aircond Services" className={inputClass} />
          </div>
          ...
          <div>
            <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">
              Pengalaman <span className="normal-case font-normal text-gray-400">(tahun)</span>
            </label>
            <input value={yearsExp} onChange={e => setYearsExp(e.target.value)} type="number" min="0" max="50"
              placeholder="cth: 5" className={inputClass} />
          </div>

          <div>
            <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">
              Bio <span className="normal-case font-normal text-gray-400">(pilihan)</span>
            </label>
            <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3}
              placeholder="Ceritakan servis yang anda tawarkan, pengalaman, dll..."
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-3 text-sm text-gray-800 outline-none focus:border-[#1565C0] transition-all font-medium resize-none" />
          </div>
          ...
          <button onClick={handleSaveProfile} disabled={saving || !name.trim() || !businessName.trim()}
